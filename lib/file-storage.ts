...
export async function saveFileMetadata(metadata: FileMetadata): Promise<void> {
  try {
    ...
    // password가 undefined가 아닌 경우에만 추가
    if (metadata.password !== undefined) {
    ...
    // 실패 시 로컬 스토리지에 저장 (백업)
    saveFileMetadataLocal({ ...metadata, expiresAt: new Date(Date.now() + 5 * 60 * 1000).toISOString() })
  }
}

function saveFileMetadataLocal(metadata: FileMetadata): void {
  if (typeof window !== "undefined") {
    const existingFiles = getStoredFilesLocal()
...
function getStoredFilesLocal(): FileMetadata[] {
  if (typeof window !== "undefined") {
    const stored = localStorage.getItem("uploadedFiles")
...
export function verifyFilePassword(file: FileMetadata, inputPassword: string): boolean {
